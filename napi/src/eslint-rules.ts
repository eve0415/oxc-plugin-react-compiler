...
const makeRule = (ruleDef: RuleDefinition): Rule.RuleModule => ({
  ...
  create(context: Rule.RuleContext): Rule.RuleListener {
    ...
    // eslint-disable-next-line typescript-eslint/no-unsafe-type-assertion -- validated by ESLint schema
    ...
    for (const diag of diagnostics) {
      ...
      if (diag.startLine === undefined || diag.startColumn === undefined) {
        continue;
      }
...
const noUnusedDirectivesRule: Rule.RuleModule = {
  ...
  create(context: Rule.RuleContext): Rule.RuleListener {
    ...
    for (const diag of diagnostics) {
      ...
      const [firstSuggestion] = diag.suggestions;
      ...
      context.report({
        ...
        suggest:
          firstSuggestion?.op === 'remove'
            ? [
                {
                  desc: 'Remove the directive',
                  fix: (fixer: Rule.RuleFixer) => fixer.removeRange([firstSuggestion.rangeStart, firstSuggestion.rangeEnd]),
                },
              ]
            : [],
      });
