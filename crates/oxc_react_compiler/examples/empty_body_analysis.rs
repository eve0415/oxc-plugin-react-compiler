fn main() {
    ...
    for entry in std::fs::read_dir(fixture_dir).unwrap() {
        ...
        // Check if output has empty arrow/function bodies
        if result.code.contains("=> {}")
            || result.code.contains("=> {\n}")
            || result.code.contains("() => {  }")
        {
            count += 1;
            // Check if expected code also has empty bodies (meaning it's correct)
            if expected_code.contains("=> {}") || expected_code.contains("=> {  }") {
                // Expected also has empty bodies — not our problem
            } else {
                // We have empty bodies but expected doesn't — this is a bug
                if count <= 5 {
                    eprintln!("BUG: {} — empty body in output but not in expected", stem);
                }
            }
        }
    }
    eprintln!("\nFixtures with empty arrow/function bodies: {}", count);
}
