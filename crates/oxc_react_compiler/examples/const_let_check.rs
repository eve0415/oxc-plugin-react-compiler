fn main() {
    ...
    for entry in std::fs::read_dir(fixture_dir).unwrap() {
        ...
        // Try with const/let normalization
        let actual_n = normalize_const_let(&actual);
        let expected_n = normalize_const_let(&expected);
    ...
    eprintln!("\nFixable by normalizing const/let: {}", fixable);
}

fn normalize_const_let(code: &str) -> String {
    code.lines()
        .map(|line| {
