fn main() {
    ...
    for entry in std::fs::read_dir(fixture_dir).unwrap() {
        ...
        // Try normalizing arrow function format
        let actual_normalized = normalize_arrows(&actual);
        let expected_normalized = normalize_arrows(&expected);

        if actual_normalized == expected_normalized {
            fixable += 1;
            if fixable <= 10 {
                eprintln!("FIXABLE: {}", stem);
            }
        }
    }
    eprintln!("\nFixable by normalizing arrows: {}", fixable);
}

fn normalize_arrows(code: &str) -> String {
    // Convert `=> {\n  return expr;\n}` to `=> expr`
    let mut result = code.to_string();
    // This is a simplistic approach - look for patterns
    while let Some(start) = result.find("=> {\nreturn ") {
        let after = &result[start + 12..]; // after "=> {\nreturn "
        if let Some(semi) = after.find(";\n}") {
            let expr = &after[..semi];
            if !expr.contains('\n') {
                let end = start + 12 + semi + 3;
