fn main() {
    ...
    for entry in std::fs::read_dir(fixture_dir).unwrap() {
        ...
        for i in 0..max {
            ...
            if a != e {
                diffs.push((a.to_string(), e.to_string()));
            }
        }

        if diffs.len() >= 4 && diffs.len() <= 6 {
            results.push((stem, diffs));
        }
    }

    results.sort_by_key(|x| x.0.clone());
    eprintln!("4-6 diff fixtures: {}", results.len());
    for (stem, diffs) in &results {
        eprintln!("\n=== {} ({} diffs) ===", stem, diffs.len());
        for (a, e) in diffs {
            eprintln!("  A: {}", a);
            eprintln!("  E: {}", e);
        }
