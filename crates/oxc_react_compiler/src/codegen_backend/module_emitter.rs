...
fn wrap_hook_guard_body<'a>(
    ...
    let mut try_statements = builder.vec1(build_hook_guard_statement(
        ...
        crate::reactive_scopes::codegen_ast::HOOK_GUARD_PUSH,
    ));
    ...
    let finalizer = builder.alloc_block_statement(
        ...
        builder.vec1(build_hook_guard_statement(
            builder,
            &state.hook_guard_ident,
            crate::reactive_scopes::codegen_ast::HOOK_GUARD_POP,
        )),
    );
...
fn build_fast_refresh_reset_statement<'a>(
    ...
    cache_binding_name: &str,
    cache_size: u32,
    fast_refresh: &crate::reactive_scopes::codegen_ast::FastRefreshPrologue,
) -> ast::Statement<'a> {
...
fn build_memo_cache_sentinel_expression<'a>(builder: AstBuilder<'a>) -> ast::Expression<'a> {
    builder.expression_call(
        ...
        builder.vec1(ast::Argument::from(builder.expression_string_literal(
            SPAN,
            builder.atom(crate::reactive_scopes::codegen_ast::MEMO_CACHE_SENTINEL),
            None,
        ))),
        false,
    )
...
fn build_early_return_sentinel_expression<'a>(builder: AstBuilder<'a>) -> ast::Expression<'a> {
    builder.expression_call(
        ...
        builder.vec1(ast::Argument::from(builder.expression_string_literal(
            SPAN,
            builder.atom(crate::reactive_scopes::codegen_ast::EARLY_RETURN_SENTINEL),
            None,
        ))),
        false,
    )
...
fn apply_internal_blank_line_markers(code: &str) -> String {
    let marker_line = format!(
        "\"{}\";",
        crate::reactive_scopes::codegen_ast::INTERNAL_BLANK_LINE_MARKER
    );
...
const z = __REACT_COMPILER_FLOW_CAST__<Array<number>>([]);"#;
    ...
    fn applies_internal_blank_line_markers_inside_blocks() {
        let marker = crate::reactive_scopes::codegen_ast::INTERNAL_BLANK_LINE_MARKER;
        let source =
