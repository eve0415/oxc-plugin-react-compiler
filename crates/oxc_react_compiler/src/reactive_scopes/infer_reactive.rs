...
/// Check whether a function has any reactive values that need memoization.
/// This is a quick check that doesn't modify the HIR.
pub fn has_reactive_values(func: &HIRFunction) -> bool {
    // A function needs memoization if:
    // 1. It has parameters (props can change between renders)
    // 2. It calls hooks (hooks access reactive state/context)
    if !func.params.is_empty() {
        return true;
    }
    // Build name lookup for hook detection
    let mut id_to_name: HashMap<IdentifierId, String> = HashMap::new();
    for (_, block) in &func.body.blocks {
        for instr in &block.instructions {
            match &instr.value {
    ...
    for (_, block) in &func.body.blocks {
        for instr in &block.instructions {
            match &instr.value {
                ...
                InstructionValue::MethodCall { property, .. } => {
                    if is_hook_identifier(property)
                        || is_use_operator_identifier(property)
                        || id_to_name
                            .get(&property.identifier.id)
                            .is_some_and(|n| is_hook_name(n) || is_use_operator_name(n))
