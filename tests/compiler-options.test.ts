...
describe('compiler option helpers', () => {
  describe('isFilePartOfSources', () => {
    it('defaults to skipping node_modules files', () => {
      expect(isFilePartOfSources('src/Component.tsx')).toBe(true);
      expect(isFilePartOfSources('node_modules/pkg/index.js')).toBe(false);
    });
  ...
  describe('withDetectedReanimatedSupport', () => {
    it('injects the reanimated environment flag when enabled and detected', () => {
      const options = withDetectedReanimatedSupport({ enableReanimatedCheck: true, environment: { enableFire: true } }, () => true);
    ...
    it('does not inject the flag when disabled', () => {
      const options = withDetectedReanimatedSupport({ enableReanimatedCheck: false, environment: {} }, () => true);
