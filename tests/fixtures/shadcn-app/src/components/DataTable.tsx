...
export function DataTable() {
  ...
  const toggleSelectAll = useCallback((users: User[]) => {
    setSelectedIds(prev => {
  ...
  const pagedUsers = useMemo(() => sortedUsers.slice(page * pageSize, (page + 1) * pageSize), [sortedUsers, page]);
  ...
  const SortHeader = useCallback(
    ({ label, field }: { label: string; field: SortKey }) => (
      <TableHead>
        <button className='flex items-center space-x-1 hover:text-foreground' onClick={() => handleSort(field)}>
          <span>{label}</span>
