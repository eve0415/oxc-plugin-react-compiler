...
export function Accordion({ type = 'single', children, className }: { type?: 'single' | 'multiple'; children: ReactNode; className?: string }) {
...
export function AccordionTrigger({ value, children, className }: { value: string; children: ReactNode; className?: string }) {
  ...
  return (
    <h3 className='flex'>
      <button className={cn('flex flex-1 items-center justify-between py-4 font-medium transition-all hover:underline', className)} onClick={handleClick}>
        {children}
        <span className={cn('h-4 w-4 shrink-0 transition-transform', isOpen && 'rotate-180')}>▾</span>
      </button>
    </h3>
  );
}
